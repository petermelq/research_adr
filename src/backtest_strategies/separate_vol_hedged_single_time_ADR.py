...
class separate_vol_hedged_single_time_ADR(BaseStrategy):
    """
    ...
    def generate_trades(self,
                        ...
                        adr_signal_for_vol: pd.DataFrame,
        ...
        if (not self.ny_normal_close(trading_day) or
            ...
            adr_signal_for_vol = adr_signal_for_vol[cols]
            adr_signal_for_vol = adr_signal_for_vol.dropna(how='all',axis=1) # dropping columns that are all nan (adrs that don't exist yet)
            ...
            res = ret - adr_signal_for_vol.iloc[-self.vol_lookback-1:-1]
            ...
            res = res[adr_signal_for_vol.columns] # making sure columns are aligned
