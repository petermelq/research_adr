...
if __name__ == '__main__':
    futures_filenames = [
        os.path.join(SCRIPT_DIR, '..', 'data', 'processed', 'futures', 'active', 'bbo-1m', 'exchange=IFLL.IMPACT', 'code=Z'),
        os.path.join(SCRIPT_DIR, '..', 'data', 'processed', 'futures', 'active', 'bbo-1m', 'exchange=XEUR.EOBI', 'code=FESX'),
        os.path.join(SCRIPT_DIR, '..', 'data', 'processed', 'futures', 'active', 'bbo-1m', 'exchange=GLBX.MDP3', 'code=NIY'),
    ]
    fx_filenames = [
    ...
    for futures_filename, fx_filename, notional_multiplier in zip(futures_filenames,
        ...
        futures_df = pl.read_parquet(
            futures_filename,
            #new_columns=['ts_recv','bid_px_00','ask_px_00']
        )
        
        # Apply notional multiplier and parse timestamp (format: YYYY-MM-DD HH:MM:SS)
        futures_df = futures_df.with_columns([
            pl.col('ts_recv').dt.replace_time_zone('America/New_York'),
            (pl.col(['bid_px_00','ask_px_00']) * notional_multiplier)
        ]) # Perform the join efficiently with polars
        futures_df = futures_df.join(fx_df, left_on='ts_recv', right_on='timestamp', how='left')
        
        # Apply FX conversion
        futures_df = futures_df.with_columns([
            (pl.col(['bid_px_00','ask_px_00']) * pl.col('fx_rate'))
        ])
        # compute mid price
        futures_df = futures_df.with_columns([
            ((pl.col('bid_px_00') + pl.col('ask_px_00')) / 2).alias('close')
        ])
        futures_df = futures_df.rename({'symbol':'contract_symbol','ts_recv':'timestamp'})
