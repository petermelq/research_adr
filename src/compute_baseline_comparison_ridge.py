"""
Compute baseline comparison for test stocks using Ridge regression.

Compares:
1. Baseline model: predicted_return = beta * index_return
2. Ridge model: predicted_return = beta * index_return + Ridge(russell_residuals)
...
test_model_dir = data_dir / 'processed' / 'models' / 'with_us_stocks' / 'ridge_test'
...
def main():
    ...
    for _, row in metadata.iterrows():
        ...
        # Load model (returns a dict with 'model' key containing the RidgeResidualModel)
        ...
        # Get Ridge residual predictions
        ...
        ridge_residual_pred = model.model.predict(X_scaled)

        # Compute predictions
        # Baseline: predicted = beta * index_return
        baseline_pred = beta_vals * index_ret

        # Ridge: predicted = beta * index_return + Ridge(russell_residuals)
        ridge_full_pred = beta_vals * index_ret + ridge_residual_pred

        # Remove any NaNs
        valid_mask = ~(np.isnan(y_actual) | np.isnan(baseline_pred) | np.isnan(ridge_full_pred))
        y_actual_clean = y_actual[valid_mask]
        baseline_pred_clean = baseline_pred[valid_mask]
        ridge_full_pred_clean = ridge_full_pred[valid_mask]
        ...
        ridge_ic = np.corrcoef(ridge_full_pred_clean, y_actual_clean)[0, 1]

        # Handle NaN correlations (happens when std=0)
        if np.isnan(baseline_ic):
            baseline_ic = 0.0
        if np.isnan(ridge_ic):
            ridge_ic = 0.0

        ic_improvement = ridge_ic - baseline_ic
    ...
    output_file = test_model_dir / 'baseline_comparison_ridge.csv'
    ...
    print(f"  Mean Ridge IC: {results_df['ridge_ic'].mean():.4f}")
    ...
    for ticker in TEST_STOCKS:
        ...
        if len(ticker_data) > 0:
            print(f"  {ticker}: Baseline={ticker_data['baseline_ic'].mean():.3f}, "
                  f"Ridge={ticker_data['ridge_ic'].mean():.3f}, "
